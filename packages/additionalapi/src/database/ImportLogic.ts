...
import {ImportData} from "./AdditionalQueries";
...
async function storeNodes(client: PoolClient, nodes: LionWebJsonNode[]) : Promise<void> {
...
export async function performImport(client: PoolClient, imports: ImportData[]) : Promise<boolean> {
    let totalNodes: LionWebJsonNode[] = []
    console.log(`performImport ${imports.length}`)
    imports.forEach(el => {
        console.log(`el.treeData keys: ${Object.keys(el.treeData)}`)
        console.log(`nodes in this import element: ${el.treeData.nodes.length}`)
        totalNodes = totalNodes.concat(el.treeData.nodes)
    })
    console.log(`performImport - array built ${totalNodes.length}`)
    if (totalNodes.length == 0) {
        return true
    }
    await storeNodes(client, totalNodes)
    console.log("store nodes returned")
    // TODO: QUERY TO ATTACH ROOTS
    // The child to be attached is the first node in each ImportData
